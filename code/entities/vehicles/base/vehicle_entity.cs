...
public partial class Vehicle_entity : Prop, IUse
{
	...
	private Vehicle_wheel frontLeft;
	private Vehicle_wheel frontRight;
	private Vehicle_wheel backLeft;
	private Vehicle_wheel backRight;
	...
	private float flipRay = 45f;
	private float grip;
	private TimeSince timeSinceDriverLeft;

	private float forwardFront = 47.75f; //42;
	private float forwardRear = 52.0f;
	private float right = 34; //32
	...
	public override void Spawn()
	{
		...
		var modelName = "models/vehicles/lanciadelta/lancia.vmdl"; //modelName = "models/car/car.vmdl";
		SetModel( modelName );
		...
		//trigger.Transform = Transform;//new Transform( new Vector3( 0, 0, -4.5f ) );
		//trigger.Scale = 1.05f;
	}
	...
	public override void ClientSpawn()
	{
		...
		var wheelScale = 0.825f;

		/*
		var vehicle_model = new ModelEntity();
		vehicle_model.SetModel( "models/vehicles/lanciadelta/lancia.vmdl" );
		vehicle_model.Transform = Transform;
		vehicle_model.Parent = this;
		vehicle_model.LocalPosition = new Vector3( 2, 0, 26 );
		vehicle_model.LocalRotation = Rotation.From( 0, 0, 0 );
		vehicle_model.LocalScale = 1f;
		*/

		//vehicle_model.LocalPosition = new Vector3( 6, -24.5f, 6 );
		//vehicle_model.LocalRotation = Rotation.From( 0, 90, 0 );
...

		{
			...
			chassis_axle_front.LocalPosition = new Vector3( 1.05f, 0, 0.35f ) * (forwardFront - 2f);

			{
				...
				//wheel0_steering.LocalPosition = new Vector3( -4f, 0f, 0f );
				{
					...
					wheel0.LocalScale = wheelScale * 4f;
				}
			}

			{
				...
				//wheel1_steering.LocalPosition = new Vector3( 4f, 0f, 0f );
				{
					...
					wheel1.LocalScale = wheelScale * 4f;
				}
			}
...

		{
			...
			chassis_axle_rear.LocalPosition = new Vector3( -1.05f, 0, 0.35f ) * (forwardRear - 2f);
	...
	public void OnPrePhysicsStep()
	{
		...
		if ( canAirControl && (airRoll != 0 || airTilt != 0) )
		{
			...
			var tr = Trace.Ray( s, s + rotation.Up * (flipRay * Scale) )
				.Ignore( this )
				.Run();
	...
	private void RaycastWheels( Rotation rotation, bool doPhysics, out bool frontWheels, out bool backWheels, float dt )
	{
		...
		var frontLeftPos = rotation.Forward * forwardFront + rotation.Right * right + rotation.Up * ((length - 10 - frontHeight));
		var frontRightPos = rotation.Forward * forwardFront - rotation.Right * right + rotation.Up * ((length - 10 - frontHeight));
		var backLeftPos = -rotation.Forward * forwardRear + rotation.Right * right + rotation.Up * ((length - 10 - rearHeight));
		var backRightPos = -rotation.Forward * forwardRear - rotation.Right * right + rotation.Up * ((length - 10 - rearHeight));
	...
	public void OnFrame()
	{
		...
		float frontOffset = (-frontHeight + 15f) - Math.Min( frontLeftDistance, frontRightDistance );
		float backOffset = (-rearHeight + 13.5f) - Math.Min( backLeftDistance, backRightDistance );

		float frontAxleOffset = frontLeftDistance - frontRightDistance; //TRY Math.Min / Math.Max for rotation as well. See if anything nice/useful happens.
		float rearAxleOffset = backLeftDistance - backRightDistance; //TRY Math.Min / Math.Max for rotation as well. See if anything nice/useful happens.
		...
		chassis_axle_front.SetBoneTransform( "Axle_front_Center", new Transform( (Vector3.Up * 0.95f) * frontOffset.Clamp( -15, 3.75f ) ), false );
		chassis_axle_rear.SetBoneTransform( "Axle_Rear_Center", new Transform( (Vector3.Up * 0.95f) * backOffset.Clamp( -15, 3.75f ) ), false );
		chassis_axle_front.LocalRotation = axleRotFront / (1.25f + (frontAxleOffset / 4000));
		chassis_axle_rear.LocalRotation = axleRotRear / (1.25f + (rearAxleOffset / 4000));
	...
	public bool OnUse( Entity user )
	{
		if ( user is SandboxPlayer player && player.Vehicle == null && timeSinceDriverLeft > 1.0f )
		{
			...
			player.Parent = this;
			player.LocalPosition = Vector3.Up * 10 - (Vector3.Right * 15);
			player.LocalRotation = Rotation.Identity;
			player.LocalScale = 0.75f;
