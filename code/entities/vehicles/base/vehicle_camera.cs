...
public class Vehicle_camera : Camera
{
	protected virtual float MinFov => 80.0f;
	protected virtual float MaxFov => 110.0f;
	protected virtual float MaxFovSpeed => 1500.0f;
	...
	protected virtual float OrbitHeight => 28.0f;
	...
	protected virtual float FirstPersonPitch => 12.5f;
	...
	public override void Activated()
	{
		...
		var car = (pawn as SandboxPlayer)?.Vehicle as Vehicle_entity;
	...
	private void DoFirstPerson()
	{
		...
		Pos = pawn.EyePos - (Vector3.Up * 14);
		Rot = pawn.Rotation * (orbitYawRot * orbitPitchRot);

		Viewer = pawn;
	}

	private void DoThirdPerson( Vehicle_entity car, PhysicsBody body )
	{
		Rot = orbitYawRot * orbitPitchRot;

		var carPos = (car.Position + (Vector3.Up * 0)) + car.Rotation * (body.LocalMassCenter * car.Scale);
		var startPos = carPos;
